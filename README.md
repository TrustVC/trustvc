...
**Endorsement chain** — pass the `TrustVCToken` address to the existing helper (auto-detects ObligationEscrow):

```ts
import { fetchEndorsementChain } from '@trustvc/trustvc';

const chain = await fetchEndorsementChain(obligationRegistry, tokenId, provider, encryptionId);
```
