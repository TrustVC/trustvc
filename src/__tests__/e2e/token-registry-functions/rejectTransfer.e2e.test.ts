...
providers.forEach(({ ethersVersion, titleEscrowVersion }) => {
  describe(`Reject Transfer Functions E2E Tests -with ethers ${ethersVersion} and token registry ${titleEscrowVersion}`, async function () {
    ...
    before(async function () {
      ...
      const _signers = signers as (ethers.Wallet | ethersV6.Wallet | SignerWithAddress)[];
      deployer = _signers[0]!;
      owner = _signers[1]!;
      newOwner = _signers[2]!;
      holder = _signers[3]!;
      beneficiary = _signers[4]!;
      ...
      const params: MintTokenParams = {
        beneficiaryAddress: beneficiary.address, //keeping both initial holder and beneficiary same
    ...
    describe('rejectTransferHolder', () => {
      describe('Successful Rejection of Holder', () => {
        beforeEach(async () => {
          await new Promise((resolve) => setTimeout(resolve, 1000));
          ...
          const params = {
            holderAddress: addresses.newHolder,
            remarks: 'Transfer Holder',
          };
          ...
          if (prevHolder == ZeroAddress) {
            const tx0 = await transferHolder(contractOptions, holder as any, params, options);
            await tx0.wait();
          }
        });
        ...
        it('should reject holder transfer successfully with remarks provided', async () => {
          const contractOptions = {
            titleEscrowAddress: addresses.titleEscrow,
          };
          ...
          const tx = await rejectTransferHolder(contractOptions, newHolder, params, options);
