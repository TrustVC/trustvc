...
export const acceptObligationRegistry = async (
  contractOptions: ObligationRegistryContractOptions,
  signer: Signer | SignerV6,
  params: AcceptObligationRegistryParams,
  ...
  try {
    ...
    if (isV6) {
      await obligationEscrowContract.accept.staticCall(encryptedRemarks);
    } else {
      await obligationEscrowContract.callStatic.accept(encryptedRemarks);
    }
  } catch (e) {
  ...
  return await obligationEscrowContract.accept(encryptedRemarks, txOptions);
};
