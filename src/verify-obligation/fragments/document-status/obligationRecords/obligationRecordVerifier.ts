...
/** Shared credentialStatus type string for classic ETR and BoE obligation records. */
...
const skip: ObligationRecordsVerifierType['skip'] = async () => {
  return {
    ...
    reason: {
      ...
      message:
        'Document is not an Obligation Records (BoE) document — classic ETR / other status skipped',
    },
  };
};
...
const test: ObligationRecordsVerifierType['test'] = (
  ...
  if (
    ...
    credentialStatuses.every((cs) => {
      ...
      // BoE / obligation only. Classic ETR (no obligationRegistry) → skip.
      // Docs with both registries still enter verify() for mutual-exclusivity ERROR.
      return (
        status?.type === TRANSFERABLE_RECORDS_TYPE && typeof status?.obligationRegistry === 'string'
      );
    })
  ) {
    return true;
